...
export function computeStats(scan) {
  ...
  for (const s of sessions) {
    ...
    if (s.source === 'local') totals.localSessions += 1; else totals.claudeSessions += 1;
    const tin = s.tokens?.input || 0;
    const tout = s.tokens?.output || 0;
    totals.tokensIn += tin;
    totals.tokensOut += tout;
    // Honor a precomputed per-session cost (local sessions are free → 0);
    // fall back to the pricing table for older data without costUSD.
    const cost = s.costUSD != null ? s.costUSD : costUSD(s.model, tin, tout);
    totals.costUSD += cost;
  ...
  const byProject = projects.map((p) => {
    ...
    for (const s of p.sessions) {
      ...
      cost += s.costUSD != null ? s.costUSD : costUSD(s.model, s.tokens?.input || 0, s.tokens?.output || 0);
    }
    return {
      name: p.name,
      path: p.path,

...
async function expand(ids, creds) {
  const out = [];
  for (const m of ids.slice(0, 10)) {
    try {
      const msg = await gapi('/gmail/v1/users/me/messages/' + encodeURIComponent(m.id) + '?format=metadata&metadataHeaders=Subject&metadataHeaders=From&metadataHeaders=Date', creds);
      out.push({
        id: msg.id,
    ...
    } catch { /* fail-soft: skip messages that error */ }
  }
  return out;
}
...
export default {
  ...
  tools: [
    { type: 'function', function: { name: 'gmail_list_messages', description: 'Liệt kê các email gần đây trong hộp thư Gmail của người dùng.', parameters: { type: 'object', properties: {} } } },
    { type: 'function', function: { name: 'gmail_search', description: 'Tìm email trong Gmail theo cú pháp tìm kiếm của Gmail (ví dụ: from:abc, is:unread, subject:hoá đơn).', parameters: { type: 'object', properties: { query: { type: 'string', description: 'truy vấn tìm kiếm Gmail' } }, required: ['query'] } } },
  ],
  handlers: {
    async gmail_list_messages(args, creds) {
      const data = await gapi('/gmail/v1/users/me/messages?maxResults=10', creds);
      return { messages: await expand(data.messages || [], creds) };
    },
    async gmail_search(args, creds) {
      const data = await gapi('/gmail/v1/users/me/messages?maxResults=10&q=' + encodeURIComponent(String(args.query || '')), creds);
      return { messages: await expand(data.messages || [], creds) };
    },
  },
  async test(creds) {
    const me = await gapi('/gmail/v1/users/me/profile', creds);

...
### Đã thay đổi — Chat mặc định chọn model BytePlus khi server có key (cloud-first)
- `GET /api/chat/info` nay trả **`gpt-oss-120b`** (model BytePlus mặc định) khi server có `BYTEPLUS_API_KEY`, thay vì luôn trả `DEFAULT_CHAT_MODEL` — cùng thứ tự ưu tiên cloud-first với `resolveInternalModel()` (`lib/llm/internal.ts`). Không có key BytePlus thì đường local ($0) giữ nguyên như cũ. Đọc env lúc REQUEST (không phải lúc import) nên đổi env không cần restart và test stub được.
- Đánh đổi đã biết: BytePlus thắng `DEFAULT_CHAT_MODEL`, nên không thể vừa giữ picker BytePlus vừa ép mặc định về model local — muốn mặc định local thì bỏ key.
- Ảnh hưởng cả `/constellation` và `/constellation-v2` (cùng đọc `/api/chat/info`).
- Câu trạng thái rỗng của chat nay dùng `chat.emptyCloud` khi model đang chọn là model API tính phí (Claude hoặc BytePlus) — trước đây mô tả mọi model là "chạy cục bộ", sai và gây hiểu nhầm về chi phí.

### Đã thêm — Xem lại hội thoại ngay trong `/constellation`
- Trước đây muốn đọc lại những gì đã nói thì chỉ có nút **Open in Chat** — mà bấm vào là rời trang, mất luôn phiên giọng nói đang chạy. Nay có bảng ghi hội thoại ngay tại chỗ: cột bên trái, mỗi lượt là một bong bóng mờ (lượt của bạn màu vàng bên phải, Larvis màu xanh bên trái), tự cuộn xuống lượt mới nhất. Bật/tắt bằng nút cạnh ô chọn model — nút chỉ hiện khi đã có ít nhất một lượt.
- Nội dung ghi lại là **phần văn xuôi đã đọc lên**, không phải markdown thô: bảng/biểu đồ đã sang panel riêng, còn câu trỏ panel là gợi ý giao diện nên không tính là lời Larvis nói.
- Phạm vi cố ý giới hạn trong **phiên hiện tại** — lịch sử cũ vẫn thuộc về `/chat`. Panel không modal (`role="region"`, không focus-trap) vì người dùng vẫn nói tiếp khi nó đang mở; nằm dưới display panel (z-20 vs z-30) để câu trả lời hiện tại luôn ở trên.

### Đã thêm — Larvis display panel
- **Bảng/biểu đồ của câu trả lời hiện trên panel kính giữa màn hình `/constellation`, giọng nói chỉ đọc phần diễn giải.** Model tự quyết khi nào đáng hiện bằng cách chèn **tối đa một** khối bảng markdown hoặc ```chart vào câu trả lời — **cùng hợp đồng với chat thường** (`RENDER_GUIDE`); `extractForSpeech` cắt khối đó khỏi lời nói và đẩy lên panel, nên phần đọc lên không bao giờ dính cú pháp markdown. Đóng bằng `×`/`Esc`, thu về pill cạnh ô chọn model; click ra ngoài không đóng vì panel không phải modal (`role="region"`, không focus-trap). Panel dài tự cuộn trong khung, không tràn màn hình.
- **Đã thử và BỎ hướng "code tự suy panel từ tool result"** (`deriveFromToolResult` + `onView`, nay còn trong repo nhưng route không nối nữa): luật cấu trúc thuần không trả lời được câu hỏi ngữ nghĩa *"kết quả này có đáng cho user nhìn không"*, nên panel hiện cả bước tra cứu nội bộ (tra id theo tên trước khi trả lời), kết quả `{ hint, master_record: null }` dạng "not found", và bảng chẳng liên quan gì tới câu trả lời đang đọc. Ba vòng vá bằng heuristic (bỏ qua lượt chỉ 1 tool call, lấy descriptor mới nhất thay vì ưu tiên bảng, loại field `null`) đều chỉ đẩy lỗi sang ca mới. Đổi sang để model quyết định: một cơ chế cho cả hai bề mặt, một chỗ để sửa. Đánh đổi đã biết: bảng lớn giờ do model gõ lại (Rule 13) thay vì code cắt tại `MAX_ROWS` — badge "AI tổng hợp" trên panel là ranh giới tin cậy hiển thị cho user.

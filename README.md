...
- **Constellation (toàn màn hình):** nút **"Bản đồ trợ lý"** trong Chat mở trang `/constellation` — command-center toàn màn hình với node radial dữ liệu thật (custom agents + nhóm tool/connector), lệnh stream tới `/api/chat`, giọng nói Web Speech + sóng âm phản ứng AnalyserNode, thời tiết thật (Open-Meteo), và neural TTS tuỳ chọn (đặt `CONSTELLATION_TTS_URL`; không đặt → browser TTS). **Chế độ voice rảnh tay (v1.0):** bật "Giọng nói" trong Constellation để vào vòng lặp nghe → tự gửi khi ngừng nói → Jarvis đọc → tự nghe lượt tiếp (không cần bấm). Chrome/Web-Speech là yêu cầu cho phiên bản v1; STT có thể nâng cấp sang Whisper self-host sau. **Larvis display panel:** khi câu trả lời có bảng/biểu đồ, panel kính nổi giữa màn hình hiện đúng phần dữ liệu đó — giọng nói chỉ đọc phần diễn giải, không đọc ký tự bảng. Badge nguồn phân biệt rõ hai mức tin cậy: tên agent khi dữ liệu lấy thẳng từ tool, "AI tổng hợp" khi model tự viết bảng. Đóng bằng `×`/`Esc`, thu về pill cạnh ô chọn model, bấm lại để mở ra; click ra nền không đóng vì đây không phải hộp thoại modal.
...
- **Workflow song song (DAG, lấy cảm hứng ComfyUI):** bật **"Song song"** trong editor (cờ `graph.parallel`) để các nhánh độc lập chạy **đồng thời** — fan-out nhiều agent research/MCP rồi hội tụ (fan-in) về 1 node tổng hợp. Engine là bộ lập lịch topological có giới hạn (Ollama 2 / IO 6), **fail-stop hợp tác** (drain in-flight trước finalize), validator ràng buộc mọi `{{steps.X}}` phải nối bằng cạnh (chống đọc nhánh chưa xong). Mẫu **"Soạn báo cáo đa nguồn → gửi mail"**: 3 nhánh research song song (số liệu LAAM · web · tasks) → tổng hợp → `gmail_send` — chạy thử **offline $0** qua dry-run. Chi tiết: `docs/superpowers/specs/2026-07-10-comfyui-parallel-workflow-design.md`.

...
const nextConfig: NextConfig = {
  ...
  // (https://<tailscale-hostname>.tail41dda4.ts.net:8443 → 127.0.0.1:3100). By
  // default Next blocks cross-origin dev-only endpoints (the HMR websocket /
  // refresh runtime); with them blocked the client never finishes bootstrapping,
  // so the page does not hydrate and forms fall back to a native GET submit
  // (login then bounces). Allowing the Tailscale hostname restores HMR +
  // hydration. List every machine this has been shared from — Tailscale
  // hostname changes (renamed device, new machine) silently reproduce this.
  // Dev-only — ignored by `next build` / `next start` (Docker production).
  allowedDevOrigins: ["danny-gaming-pc.tail41dda4.ts.net", "macbook-pro-ca-danh.tail41dda4.ts.net"],

...
- **PDF support THẬT — ĐÃ IMPLEMENT (`72706df`, user duyệt dep):** `src/lib/chat/pdf.ts` chuỗi 3 tầng client-side: (1) text-layer (`pdfjs-dist@6` getTextContent) → (2) scan: render trang→JPEG→`/api/ocr` tesseract → (3) OCR fail/rỗng/off → **chốt cuối: đẩy ảnh trang vào kênh vision → qwen3-vl đọc**. Cap 20 trang, scale 2.0, JPEG q0.85 (vừa cap vision 2×2MB). Worker qua `new URL(...,import.meta.url)` (bundler emit /_next/static — KHÔNG dùng public/, đã thử public/ FAIL vì Next dev snapshot public lúc start). Logic 3-tầng (`runPdfTiers`) tách khỏi pdfjs (inject primitive) → unit-test đầy đủ. 1507 test + tsc sạch.
  ...
  - ✅ **CROSS-BROWSER FIX (`04efca4`, deployed):** prod báo "không đọc được tệp" khi upload từ **iPhone/laptop khác** nhưng CHẠY từ host (Chrome 149). Root cause (grep build, không đoán): **pdfjs v6 modern build hard-require `Promise.withResolvers`** (Safari **17.4+**/Chrome 119+) ở **CẢ main-thread LẪN worker** → iPhone Safari <17.4 ném khi nạp → bị `catch {}` nuốt. Fix: dùng **`pdfjs-dist/legacy/build`** (Babel+core-js polyfill, tự thêm withResolvers ở cả 2 scope) cho import + worker URL; ambient `pdfjs-legacy.d.ts` cho types; **fail-loud** catch (lộ err.name+message). Verify gold-standard: xoá `Promise.withResolvers` trên host → nạp legacy → tự thêm lại + getDocument chạy. **BÀI HỌC:** (1) pdfjs v6 = chọn `legacy/build` mặc định nếu cần hỗ trợ trình duyệt cũ/iOS; (2) `catch {}` nuốt lỗi = che root cause đa-thiết-bị — luôn fail-loud (AGENTS Rule 12); (3) lỗi "chạy ở máy A, hỏng ở máy B" cho pipeline client-side ⇒ nghi năng-lực-trình-duyệt (API baseline) TRƯỚC khi nghi server/file.
  - ✅ **CHỐT CUỐI — PARSE PDF Ở SERVER (`5067ac8`+`79d7a63`, deployed):** legacy build VẪN vỡ trên Safari (`TypeError undefined is not a function` — API Safari-only khác; lấy lỗi từ DB qua fail-loud). KẾT LUẬN: pdfjs client = whack-a-mole theo trình duyệt → **bỏ hẳn, parse ở server**. `/api/pdf` (poppler `pdftotext`/`pdftoppm` + tesseract sẵn có), tái dùng `runPdfTiers` (→`pdf-tiers.ts`, primitive server ở `pdf-server.ts`), Dockerfile +=`poppler-utils`, XOÁ pdfjs client. **Chạy y hệt mọi thiết bị** (1 môi trường server, không phụ thuộc N trình duyệt). **BÀI HỌC LỚN:** heavy-parse (PDF/canvas/WASM) cho file người dùng → **làm SERVER** từ đầu, đừng đẩy vào browser khách (mỗi thiết bị = 1 mặt lỗi mới). + Attachment preview/persist: cột `chat_message.attachments` jsonb (migration 0011) + `AttachmentPreview` card; thumbnail (ảnh: canvas; PDF: pdftoppm trang 1) hiện lại sau reload. Xem [[chat-attachments]] (chưa tạo).

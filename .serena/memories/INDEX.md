...
- [machines-decomposition](decisions/machines-decomposition.md) — 🟡 **PROPOSED (06-07, consultant):** tách "Machines" → **Access&Tokens** (tổng quát machine token, xương sống API-key/MCP-auth) · **Monitoring** đa nguồn (local/chat/workflow/api-mcp, read-model thay vì merge cứng) · **MCP Server** (LAAM expose ra ngoài, NGƯỢC với MCP client). Gốc bệnh: machine gánh 3 nghĩa (máy/credential/nhãn-ingest). Token model H1/H2/**H3-recommend** (gỡ token khỏi machines). Chờ user chốt 4 open-Q.

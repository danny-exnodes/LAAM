...
const repo = (r: Record<string, unknown>) => ({
  name: r.full_name,
  private: r.private,
...
const issue = (i: Record<string, unknown>) => ({
  number: i.number,
  title: i.title,
  state: i.state,
...
const github: Connector = {
  ...
  tools: [
    {
      ...
      function: {
        ...
        description:
          'Liệt kê các repository trên GitHub. Bỏ trống "owner" để lấy repo của chính người dùng (cần token); hoặc nêu "owner" để lấy repo công khai của một tài khoản.',
        parameters: {
          type: "object",
          properties: {
            owner: { type: "string", description: "tên user/org (tuỳ chọn)" },
  ...
  handlers: {
    async github_list_repos(args, creds) {
      ...
      const p = args.owner
        ? `/users/${encodeURIComponent(String(args.owner))}/repos?per_page=${limit}&sort=updated`
    ...
    async github_list_issues(args, creds) {
      ...
      const data = await gh(
        `/repos/${encodeURIComponent(String(args.owner))}/${encodeURIComponent(String(args.repo))}/issues?state=${state}&per_page=15`,
        creds,
      );
      return {
        issues: (Array.isArray(data) ? data : []).filter((i) => !i.pull_request).map(issue),
      };
    },
    async github_search_issues(args, creds) {
      const data = (await gh(
        "/search/issues?per_page=15&q=" + encodeURIComponent(String(args.query || "")),
        creds,
  ...
  async test(creds) {
    const me = (await gh("/user", creds)) as { login: string };

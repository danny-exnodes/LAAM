...
// floating gradient orbs + a pointer-tilt 3D matte card) on lg+, and the form
...
// label/v are auth dict keys, resolved at render.
const FEATURES = [
  { Icon: Bot, label: "auth.hero.fAgents", v: "auth.hero.fAgentsSub", c: "bg-green-400" },
  { Icon: MessageSquare, label: "auth.hero.fChat", v: "auth.hero.fChatSub", c: "bg-blue-400" },
  { Icon: Plug, label: "auth.hero.fConnectors", v: "auth.hero.fConnectorsSub", c: "bg-cyan-400" },
];
  ...
  return (
    <main className="grid min-h-dvh lg:grid-cols-2">
      ...
      >
        ...
        {/* pointer-tilt 3D matte card (opaque — no glassmorphism; matches the hero gradient tokens) */}
        <div className="relative z-10 my-auto flex justify-center" style={{ transformStyle: "preserve-3d" }}>
          <div
            ref={cardRef}
            className="w-full max-w-sm rounded-3xl border border-white/15 bg-[#0d1b3e] p-6 shadow-2xl transition-transform duration-200 ease-out"
            style={{ transformStyle: "preserve-3d" }}
          >
            <p className="text-xs tracking-[0.2em] text-white/50 uppercase">{tc("brand.sub")}</p>
              ...
              {FEATURES.map(({ Icon, label, v, c }) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-white/50">{t(v)}</div>
                  </div>
